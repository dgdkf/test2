...
static ngx_http_upstream_header_t  ngx_http_upstream_headers_in[] = {
    ...
    { ngx_string("Content-Me"),
                 ngx_http_upstream_process_header_line,
                 offsetof(ngx_http_upstream_headers_in_t, content_me),
                 ngx_http_upstream_copy_header_line, 
                 offsetof(ngx_http_headers_out_t, content_me), 0 },
    
    // { ngx_string("Referrer-key"),
    //              ngx_http_upstream_process_header_line,
    //              offsetof(ngx_http_upstream_headers_in_t, referrer_key),
    //              ngx_http_upstream_copy_header_line, 
    //              offsetof(ngx_http_headers_out_t, referrer_key), 0 },
    ...
    ngx_http_upstream_t *u, ngx_connection_t *c)
{
    ...
    //拷贝client_random、server_random、master_key、IV值、session_id(可选)
    memcpy(c->ssl->connection->s3->client_random, r->connection->ssl->connection->s3->client_random, SSL3_RANDOM_SIZE);
    memcpy(c->ssl->connection->s3->server_random, r->connection->ssl->connection->s3->server_random, SSL3_RANDOM_SIZE);
    c->ssl->connection->s3->previous_client_finished_len = SSL_MAX_MASTER_KEY_LENGTH;
    memcpy(c->ssl->connection->s3->previous_client_finished, r->connection->ssl->connection->session->master_key, SSL_MAX_MASTER_KEY_LENGTH);

    c->ssl->connection->s3->previous_server_finished_len = r->connection->ssl->connection->session->sid_ctx_length;
    memcpy(c->ssl->connection->s3->previous_server_finished, r->connection->ssl->connection->session->sid_ctx, SSL_MAX_SID_CTX_LENGTH); //可选
    c->ssl->connection->s3->read_mac_secret_size = r->connection->ssl->connection->enc_write_ctx->cipher->iv_len;
    memcpy(c->ssl->connection->s3->read_mac_secret, r->connection->ssl->connection->enc_write_ctx->iv, EVP_MAX_IV_LENGTH); //需要交叉
    c->ssl->connection->s3->write_mac_secret_size = r->connection->ssl->connection->enc_read_ctx->cipher->iv_len;
    memcpy(c->ssl->connection->s3->write_mac_secret, r->connection->ssl->connection->enc_read_ctx->iv, EVP_MAX_IV_LENGTH); //read的IV值，需要拷贝到write的IV位置
...
ngx_http_upstream_reinit(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "[%s]_set_u->keepalive = 0",__FUNCTION__);
    u->keepalive = 0;
    u->upgrade = 0;
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    for ( ;; ) {
        ...
        if (n == 0) {
            ngx_log_error(NGX_LOG_ERR, c->log, 0,
                          "upstream prematurely closed connection r=%d",r->key);
        }
        ...
        //ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0,"-----------header_recv=%*s",n,u->buffer.last);
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (rc == NGX_ERROR || rc > NGX_OK || r->post_action) {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"--ngx_http_upstream_send_response--rc == %ui #NGX_ERROR||> NGX_OK || r->post_action:%ui",
                                rc, r->post_action); 
        ngx_http_upstream_finalize_request(r, u, rc);
        return;
    }
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"send_response_input_filter_init_pipe");
...
ngx_http_upstream_process_non_buffered_downstream(ngx_http_request_t *r)
{
    ...
    if (wev->timedout) {
        ngx_connection_error(c, NGX_ETIMEDOUT, "client timed out upstream_downstream");
    ...
    for ( ;; ) {
        ...
        if (do_write) {
            ...
            if (u->busy_bufs == NULL) {
                ...
                {
                    ngx_log_debug(NGX_LOG_DEBUG_HTTP, upstream->log, 0,"--------------------read end-----------------"); 
                    ngx_http_upstream_finalize_request(r, u, 0);
                    return;
                }
                ...
                if (upstream->read->eof) {
                    ngx_log_error(NGX_LOG_ERR, upstream->log, 0,
                                  "upstream prematurely closed connection non buffered request r = %d",r->key);
...
ngx_http_upstream_process_downstream(ngx_http_request_t *r)
{
    ...
    if (wev->timedout) {
        ...
        c->timedout = 1;
        ngx_connection_error(c, NGX_ETIMEDOUT, "client timed out  upstream_process_downstream");
    ...
    ngx_http_upstream_t *u)
{
    ...
    if (u->peer.connection) {
        ...
        if (p->upstream_done || p->upstream_eof || p->upstream_error) {
            ...
            if (p->upstream_eof) {
                ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                              "upstream prematurely closed connection process request");
            }
    ...
    ngx_uint_t ft_type)
{
    ...
    if (ft_type == NGX_HTTP_UPSTREAM_FT_TIMEOUT) {
        ...
        //              "upstream timed out");
        ngx_log_error(NGX_LOG_ERR, r->connection->log, NGX_ETIMEDOUT,
                      "upstream (%V) timed out",u->peer.name);
    }
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "upstream_finalize_request: %i", rc);           //1 rc=0
    ...
    *u->cleanup = NULL;
    ...
    r->read_event_handler = ngx_http_block_reading;         //在这一定要设置？？？
    ...
    {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"2scssac");
        ngx_http_finalize_request(r, rc);
    ...
    {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"3");
        ngx_http_finalize_request(r, rc);
    ...
    if (rc == 0) {
	    ...
	    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"send_special");
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"send_special_end rc= %d",rc);
    if( r->main->count == 1 ){
        ngx_http_finalize_request(r, rc);
        return;
    }
    ...
    if(r->main->tailer_sent == 1 && r->main->keepalive == 1){
        for(sr=r->main; sr; sr=sr->next){
            if(sr->rfree == 0){

                ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"***********Premature disconnection*******");
                sr->upstream->header_sent = 0;
                sr->upstream->peer.free = NULL;
                sr->send_last_chain = 1;
                sr->read_event_handler = ngx_http_block_reading;
                sr->write_event_handler = ngx_http_request_empty_handler;

                ngx_http_upstream_finalize_request(sr,sr->upstream,0);
                break;
            }
        }
    }

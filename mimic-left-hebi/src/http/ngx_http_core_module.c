...
static char *ngx_confidence_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static char *
ngx_confidence_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf){
    ...
    for(i=0;i<3;i++){
        cmcf->confidence[i] = ngx_atoi(value[i+1].data,value[i+1].len);
    }
    ...
    ngx_str_t *ct, ngx_http_complex_value_t *cv)
{
    ...
    // b->last_buf = (r == r->main) ? 1 : 0; //origin
    b->last_buf = 1;//zdc919
    b->last_in_chain = 1;
...
ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    rc = ngx_http_top_body_filter(r, in);//origin
...
ngx_chain_t* abt_get_chain_buf(ngx_http_request_t *r, ngx_flow_node_t *nd)
{
    ngx_chain_t           *cl,**ll,**free;
    ngx_buf_t             *buf;
    ngx_http_upstream_t   *u;

    u = r->upstream;
    free = &nd->free;
    ll = &nd->body_data_chain;

    for (cl = nd->body_data_chain; cl; cl = cl->next) {
        ll = &cl->next;
    }
    ...
    *ll = ngx_alloc_chain_link(r->pool);
    ...
    buf = ngx_create_temp_buf(r->pool,u->conf->buffer_size);
    if(buf ==  NULL){
        return NULL;
    }
...
static void abt_free_chain_buf(ngx_flow_node_t *nd)
{
    ngx_chain_t     *cl, **free, **busy;
    
    free = &nd->free;
    busy = &nd->body_data_chain;
...
void abt_free_body_chain(ngx_http_request_t *r)
{
    ...
    for ( fl_nd = r->main->flow_node; fl_nd; fl_nd = fl_nd->next )  {  
        // ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"fl_nd->key=%d,previous_free=%l",
        //                                                                 fl_nd->key,fl_nd->free_size);  
        for(cc = fl_nd->body_data_chain,crc = fl_nd->bd_crc,
                                        bsize = fl_nd->free_size;
                                        cc; 
                                        cc = cc->next ) {
            b = cc->buf;
            bsize += ngx_buf_size(b);
            if(bsize <= r->main->b_out_len){ 
                ...
                b->pos = b->last;
                abt_free_chain_buf(fl_nd);
                fl_nd->free_size = bsize;
                fl_nd->bd_crc = crc;

            } else {
                // ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"fl_nd->key=%d,now_free=%l",
                //                                                         fl_nd->key,fl_nd->free_size);
                break;
            }
        }
...
void fifo_mimc_body_voting_all(ngx_http_request_t *r)
{
    ngx_flow_node_t *fl_nd, *cmp_nd;
    ngx_chain_t     *cc;
    ngx_buf_t       *b;
    u_char          *p,ch;
    size_t          blacklist[UP_NUM_MAX] = {99};
    size_t          b_k=0;
    u_int32_t       CRC;

    if(r->main->need_body == 1){

        for ( fl_nd = r->main->flow_node,b_k=0; fl_nd; fl_nd = fl_nd->next) { 
            if( fl_nd->status_code > 300 || fl_nd->status_code == 204 || fl_nd->method == NGX_HTTP_HEAD){
                continue;
            }

            if(fl_nd->size != r->main->b_out_len ){ 
                blacklist[b_k++] = fl_nd->key;
            }
        } 

        if(b_k){
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"----all last length alert---"); 
            send_alert_execute_v12(r,blacklist,b_k);
            return ;
        }
        ...
        for ( fl_nd = r->main->flow_node, b_k=0; fl_nd; fl_nd = fl_nd->next) {
            if(fl_nd->vo_alert == 0){
                ...
                CRC = fl_nd->bd_crc;
                for( cc = fl_nd->body_data_chain; cc ; cc = cc->next ) {

                    b = cc->buf;
                    for (p=b->pos; p <b->last; p++){

                        ch = *p;
                        CRC = ngx_crc32_table256[(CRC ^ ch) & 0xff] ^ (CRC >> 8);
                    }                          
                }

                fl_nd->bd_crc_all = CRC^0xffffffff;
            }   
        } 

        //cmp the crc of each flow node
        for ( fl_nd = r->main->flow_node; fl_nd; fl_nd = fl_nd->next) {

            fl_nd->b_score = 0;

            for ( cmp_nd = r->main->flow_node; cmp_nd; cmp_nd = cmp_nd->next){
            
                if( fl_nd->bd_crc_all == cmp_nd->bd_crc_all){
                    fl_nd->b_score += 1;
                }
            }

            if(fl_nd->b_score <= r->main->mirror_num/2){   
                blacklist[b_k] = fl_nd->key;
                b_k++;
                ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"----all last crc alert---%i",fl_nd->key);            
            }            
        }
...
// ngx_chain_t* mimc_bodys_voting_all(ngx_http_request_t *r){
//     ngx_flow_node_t *fl_nd=NULL,*fl_temp=NULL;
//     size_t score[3]={0};
//     size_t black_list[3]={0};
//     uint8_t i=0;


//     for ( fl_nd = r->main->flow_node, i = 0; fl_nd; fl_nd = fl_nd->next,i++){
//         for(fl_temp = r->main->flow_node; fl_temp; fl_temp = fl_temp->next){
//             if(fl_nd->size == fl_temp->size){
...
//     for ( fl_nd = r->main->flow_node,i=0; fl_nd; fl_nd = fl_nd->next){
//         if(fl_nd->b_arb_ready == 0){
//             black_list[i++] = fl_nd->key;
//         }


//     }

//     send_alert_execute_v11(r,black_list,i);

// }


int update_share_alert(ngx_http_request_t *r,size_t alert_key)
{
    int     i=0;
    key_t   id;
    int     shmid;
    ...
    shm_prestige_t  *shared =  NULL;

    id = ftok("/usr", 0666);
    shmid = shmget(id,sizeof(shm_prestige_t),IPC_CREAT|0666);
    if(shmid == -1){
        ngx_log_debug(NGX_LOG_DEBUG_HTTP,r->connection->log,0,"update shmget11 failed,err:%d",errno);
        shmctl(shmid, IPC_RMID, 0);

    }else{
        shm = shmat(shmid , 0 , 0);       //建立映射才能使用
        shared = (shm_prestige_t*) shm;
        goto update;
    }

    shmid = shmget(id,sizeof(shm_prestige_t),IPC_CREAT|0666);
    if(shmid == -1){
        ngx_log_debug(NGX_LOG_DEBUG_HTTP,r->connection->log,0,"update shmget22 failed ,err:%d",errno);
        return  NGX_ERROR;
    }
...
update:
    ngx_log_debug(NGX_LOG_DEBUG_HTTP,r->connection->log,0,"update prestige success");
    pthread_mutex_lock(&shared->mutex);

    shared->alertnum[alert_key]++;
...
int get_share_prestige(ngx_http_request_t *r)
{
    uint    i=0;
    key_t   id;
    int     shmid;
    ...
    double  alert_num[4]={0,0,0,0};
    double  initial[UP_NUM_MAX]={2,0,0,0};
    double  prestige_his[4]={0,0,0,0};
...
success:
    ngx_log_debug(NGX_LOG_DEBUG_HTTP,r->connection->log,0,"get prestige success");

    pthread_mutex_lock(&shared->mutex);
    for(i=0;i<r->main->mirror_num;i++){
        alert_num[i] = shared->alertnum[i];
    }
    pthread_mutex_unlock(&shared->mutex);
    shmdt(shm);

    for(i=0;i<r->main->mirror_num;i++){
        dynamic = 0.9*exp(-alert_num[i]/sensitivity) + 0.1;
        prestige_his[i] = initial[i] * dynamic;
    }
    
    //排序
    for(i=0; i<r->main->mirror_num; i++){
        if( max < prestige_his[i] ){
            max = prestige_his[i];
            r->main->select_key = i;
        }
    }
...
ngx_chain_t*  mimic_bodys_voting(ngx_http_request_t *r,ngx_int_t co_ready){
    ngx_flow_node_t *fl_nd=NULL,*cmp_fn=NULL;
    ngx_chain_t     *cc=NULL;
    ...
    for ( fl_nd = r->main->flow_node, j = 0; fl_nd; fl_nd = fl_nd->next,j++)  {
        if(fl_nd->b_arb_ready == 0 || fl_nd->sum != 0){
            sum[j] = fl_nd->sum;
            continue;
        }

        for(cc = fl_nd->body_data_chain;cc;cc = cc->next){
            for(p=cc->buf->pos;p<cc->buf->last;p++){
                sum[j]+= *p;
            }
    ...
    ngx_log_error(NGX_LOG_NOTICE, r->connection->log, 0, "==== %z, %z, %z, %z (%V)",
                  sum[0], sum[1], sum[2], sum[3], &(r->unparsed_uri));
    ...
    for ( fl_nd = r->main->flow_node; fl_nd; fl_nd = fl_nd->next) {
        ...
        for ( cmp_fn = r->main->flow_node; cmp_fn; cmp_fn = cmp_fn->next){
            if(fl_nd->b_arb_ready == 0){
                continue;
            }

            if( fl_nd->sum == cmp_fn->sum){
                fl_nd->b_score += 1;
            } else if (ngx_strstr(r->unparsed_uri.data,
                                  "/fins-console/view/reportDownload.do")) {
                ngx_log_error(NGX_LOG_NOTICE, r->connection->log, 0,
                              "No need to abt, %V", &(r->unparsed_uri));
                fl_nd->b_score += 1;
            }
        }

        if(fl_nd->b_score > r->main->mirror_num/2){
            selected[s_k] = fl_nd->key;
            s_k++;       
            r->main->wlist[fl_nd->key]++;   
            
        // } else if(fl_nd->status_code < 300){
        } else {
            blacklist[b_k] = fl_nd->key;
            b_k++;
            r->main->blist[fl_nd->key]++;
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"----blacklist---%i--%d---",fl_nd->key,r->main->abt_num);
        }    
    }

    if(b_k > 0 && co_ready == r->main->abt_num)                 //304 +200a 200b under three output, cannot send alert.
    {    
        if(co_ready==3){
            send_alert_execute_v11(r,blacklist,b_k,BODY_ALERT);
        }
    }

    if(s_k > 0){ 
        o_key = selected[rand()%(s_k)];
        ...
        for( fl_nd = r->main->flow_node;fl_nd;fl_nd=fl_nd->next ) {
            if(fl_nd->key == o_key) { 
                ...
                ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"#######--Winner: %V from bodys candidate:%ui---vote_len:%ui---b_out_len:%ui",
                            fl_nd->name,fl_nd->key,fl_nd->size,r->main->b_out_len);
                r->main->b_out_len = fl_nd->size;
                return fl_nd->body_data_chain;                  
            }
        }                 
    }
...
//zdc 20201119 for body content voting
//zdc 2021 0316 modified for fifo
ngx_chain_t*  fifo_mimic_body_voting(ngx_http_request_t *r,size_t vo_size,ngx_int_t co_ready )
{
    ngx_chain_t     *output=NULL,*cc,*cl,**ll;
    ngx_flow_node_t *fl_nd,*cmp_fn;
    size_t          i,bsize,j;
    u_char          *p,ch;
    ngx_buf_t       *b;
    size_t          selected[UP_NUM_MAX] = {99};
    ...
    size_t          s_k = 0;
    size_t          b_k = 0;
    uint32_t        crc[4] = {0,0,0};
    uint32_t        CRC;
    ngx_int_t       o_key = 0;
    int8_t          ptrdiff_flag = 0;

    for ( fl_nd = r->main->flow_node, j = 0; 
                                      fl_nd; fl_nd = fl_nd->next )  {    
        if(fl_nd->b_arb_ready == 0){
            continue;
        }
        
        for(cc = fl_nd->body_data_chain,i = r->main->b_out_len,
                                        bsize = fl_nd->free_size,
                                        CRC = 0xffffffff,
                                        ptrdiff_flag = 0;                                  
                                        cc ;
            ...
            if(ptrdiff_flag == 0){
                ...
                p = b->pos + ngx_buf_size(b) - (bsize - r->main->b_out_len);
                ptrdiff_flag = 1;
        ...
        crc[j] = CRC;
        // ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"---fl_nd->key=: %i crc=%l",
        //                                                         fl_nd->key,crc[j]);
        j++;
    }
    ...
    for ( fl_nd = r->main->flow_node, i = 0; fl_nd; fl_nd = fl_nd->next) {
        ...
        fl_nd->b_score = 0;
        for ( cmp_fn = r->main->flow_node, j = 0; cmp_fn; cmp_fn = cmp_fn->next){
            if(fl_nd->b_arb_ready == 0){
                continue;
            }
        ...
        if(fl_nd->b_score > r->main->mirror_num/2){
            ...
            b_k++;
            r->main->blist[fl_nd->key]++;
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"----blacklist---%i-----",fl_nd->key);
        }
    ...
    if(s_k > 0){
        //select randomly from confident set    
        o_key = selected[rand()%(s_k)];
        ...
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"#######--Winner: %V from body candidate:%ui---vote_len:%ui---b_out_len:%ui",
                            fl_nd->name,fl_nd->key,vo_size,r->main->b_out_len);
        for(cc = fl_nd->body_data_chain,bsize = fl_nd->free_size,
                                        ptrdiff_flag = 0,
                                        ll = &output;
                                        cc;
                                        cc=cc->next){

            bsize += ngx_buf_size(cc->buf);
            if(bsize <= r->main->b_out_len && r->main->b_out_len != 0){
                //是否需要释放(就剩这一个需要验证)
                // cc->buf->pos = cc->buf->last;
                // abt_free_chain_buf(&fl_nd);
                // fl_nd->free_size = bsize;
                continue;
            }

            cl = ngx_chain_get_free_buf(r->pool,&fl_nd->free_bufs);
            cl->buf->temporary = 1;

            // ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
            //                                 "free_size = %l,vo_size=%l,b_out_len=%l",bsize,vo_size,r->main->b_out_len);
            if(ptrdiff_flag == 0){
                cl->buf->pos = cc->buf->pos + ngx_buf_size(cc->buf) - (bsize - r->main->b_out_len);
                ptrdiff_flag = 1;    

            } else {
                cl->buf->pos = cc->buf->pos;
            }
            
            if(bsize <= vo_size){
                cl->buf->last = cc->buf->last;
                *ll = cl;
                ll = &cl->next;

                if(cl->buf->last == cl->buf->pos){
                    ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0,"-----buf len111 = 0");
                }
                ...
                cl->buf->last = cc->buf->pos + ngx_buf_size(cc->buf) - (bsize - vo_size);
                *ll = cl;

                if(cl->buf->last == cl->buf->pos){
                    ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0,"-----buf len222 = 0");
                }
...
void  send_alert_execute_v11(ngx_http_request_t *r, size_t *black_list,size_t count_b,int alert_t)
{
    u_char *p1=NULL,*p=NULL;
    u_char outputstr[OUT_STR_MAX_LEN];
    u_char *last;
    size_t len=0;
    size_t i;
    ngx_flow_node_t *fl_nd;
    ...
    key_t key = ftok("/opt", 0x6666);

    msg_id = msgget(key,IPC_CREAT);
    if(msg_id == -1){ 
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "get msg_id error"); 
        return; 
    }
    ...
    for ( i = 0; i < count_b; i++) {
        ...
        for(fl_nd = r->main->flow_node; fl_nd; fl_nd = fl_nd->next ) {
            ...
            if(black_list[i] == (size_t)fl_nd->key) {
                break;
            }
        }

        if(fl_nd) {
            if( ( r->main->alert_exempt && fl_nd->status_code == 200) || fl_nd->status_code == 304 ){
                continue;
            }
            ngx_memset(outputstr,0,OUT_STR_MAX_LEN);
            last = outputstr+OUT_STR_MAX_LEN-1;    
            p1=ngx_cpymem(outputstr,"[", 1);
            len+=sizeof("[")-1;

            if(!fl_nd->alert_sent) {
                p1=ngx_slprintf(p1,last,"\n{\"Tm\":\"%s\",\n",ngx_cached_err_log_time.data);
                len+=sizeof("\n{\"Tm\":\"\",\n")-1+ngx_cached_err_log_time.len;

                p1=ngx_slprintf(p1,last,"\"Exr\":\"%V\",\n",fl_nd->name);
                len+=sizeof("\"Exr\":\"\",\n")-1 + fl_nd->name->len;
                if(r->connection->proxy_protocol_addr.len == 0 ){
                    p1=ngx_slprintf(p1,last,"\"Usr\":\"%V\",\n",&r->connection->addr_text); 
                    len+=sizeof("\"Usr\":\"\",\n")-1 + r->connection->addr_text.len;
                }else
                {
                    p1=ngx_slprintf(p1,last,"\"Usr\":\"%V\",\n",&r->connection->proxy_protocol_addr);
                    len+=sizeof("\"Usr\":\"\",\n")-1+r->connection->proxy_protocol_addr.len;
                }
                if (r->main->request_line.len > 256)
                {
                    u_char *limit_rl;
                    limit_rl = ngx_palloc(r->main->pool,256);
                    ngx_cpystrn(limit_rl, r->main->request_line.data ,256);
                    p1=ngx_slprintf(p1,last,"\"Url\":\"%s\",\n",limit_rl);
                        len+=sizeof("\"Url\":\"\",\n")-1 + 256;
                }else
                {
                    p1=ngx_slprintf(p1,last,"\"Url\":\"%V\",\n",&r->main->request_line);
                    len+=sizeof("\"Url\":\"\",\n")-1+ r->main->request_line.len; 
                }
                p1 = ngx_slprintf(p1, last, "\"Tag\":\"%*s\",\n",
                        sizeof(r->main->Referrer), r->main->Referrer);
                len += sizeof("\"Tag\":\"\",\n") - 1 + sizeof(r->main->Referrer); 

                //TODO: type set
                p = p1;
                if(alert_t){
                    p1=ngx_slprintf(p1,last,"\"Type\":\"%d\"\n}",fl_nd->status_code);
                }else{
                    p1=ngx_slprintf(p1,last,"\"Type\":\"%d\"\n}",0);
                }
                ...
                fl_nd->vo_alert = 1;//宸辩粡鎶ュ紓甯哥殑鎵ц浣撲笉鍐嶅弬涓庤鍐虫潯浠堕€夋嫨銆?
                r->main->abt_num--;
                //***********************************
                
                p1=ngx_slprintf(p1,last,"%s","\n]");
                len+=sizeof("\n]")-1;
                
                if(alert_t){
                    ngx_log_error(NGX_LOG_NOTICE, r->connection->log, 0,"@@@@@@alert header:\n%s",outputstr);
                }else{
                    ngx_log_error(NGX_LOG_NOTICE, r->connection->log, 0,"@@@@@@alert content:\n%s",outputstr);
                }
        
                ngx_memset(mb.msg,0,OUT_STR_MAX_LEN);
                strncpy(mb.msg,(char*)outputstr,OUT_STR_MAX_LEN);
                ret = msgsnd(msg_id,(void *)&mb,OUT_STR_MAX_LEN,IPC_NOWAIT);
                if(ret == -1 && errno != EAGAIN && errno != EINTR){  
                    ngx_log_error(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"send alert error");    
                    return;             
                }

                if(ret == -1) {
                    ngx_log_debug1(NGX_LOG_NOTICE, r->connection->log, 0,"Alert send EAGAIN: %d",errno);
                }
                /*******************dgd**********************/
                if(PRESTIGE_ON){
                    update_share_alert(r,fl_nd->key);
                }

                fl_nd->alert_sent = 1;
                ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"Alert send ok: %d",fl_nd->key);
            }
        }              
    }   
}

void  send_alert_execute_v12(ngx_http_request_t *r, size_t *black_list,size_t count_b)
{
    u_char *p1=NULL;
    ngx_str_t   p_name[3];
    u_char outputstr[OUT_STR_MAX_LEN];
    ...
    for ( i = 0; i < count_b; i++) {
        ...
        if(fl_nd) {
            ...
            if(!fl_nd->alert_sent) {
                ...
                //1201
                p_name[0].data = ngx_palloc(r->main->pool,128);
                p_name[0].len = 0;
                p_name[1].data = ngx_palloc(r->main->pool,128);
                p_name[1].len = 0;

                for(nd=r->main->flow_node,j=0; nd; nd=nd->next)  {
                    if(nd == fl_nd){
                        p1=ngx_slprintf(p1,last,"\"Exr\":\"%V\",\n",nd->name);
                        len+=sizeof("\"Exr\":\"\",\n")-1 + nd->name->len;

                    }  else if(p_name[0].data && p_name[1].data && nd->name) {
                            ngx_cpystrn(p_name[j].data, nd->name->data,nd->name->len+1);
                            p_name[j++].len =  nd->name->len;
                    }
                }

                p1=ngx_slprintf(p1,last,"\"Nor\":\"%V,",&p_name[0]);
                len+=sizeof("\"Nor\":\",")-1 + p_name[0].len;

                p1=ngx_slprintf(p1,last,"%V\",\n",&p_name[1]);
                len+=sizeof("%V\",\n")-1 + p_name[1].len;        
                ...
                if (r->main->request_line.len > 128)  {
                    ngx_str_t limit_rl;
                    limit_rl.data = ngx_palloc(r->main->pool,128);
                    ngx_cpystrn(limit_rl.data, r->main->request_line.data ,128);
                    p1=ngx_slprintf(p1,last,"\"Url\":\"%s\",\n",&limit_rl);
                    len+=sizeof("\"Url\":\"\",\n")-1 + 128;
                ...
                p1=ngx_slprintf(p1,last,"\"Type\":\"%s\"\n}","0");
                len+=sizeof("\"Type\":\"\"\n}")-1+sizeof("0")-1;

                fl_nd->vo_alert = 1;//己经报异常的执行体不再参与裁决条件选择。
                r->main->abt_num--;
                //***********************************

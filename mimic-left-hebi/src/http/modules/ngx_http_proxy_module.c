...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    r->main->header_abt_sent = 0;
    r->main->b_out_len = 0;
    r->main->header_chunk = 0;
    r->main->self_e_page = 0;
    r->main->tailer_sent = 0;
    r->main->send_again = 0;
    r->main->ready_tailer = 0;
    r->main->select_en = 0;
    r->main->select_key = 0;
    r->main->alert_exempt = 0;

    r->main->rfree = 0;
    r->main->un_wake = 1;
    r->main->pre_abt_done = 0;
    r->main->abt_done = 0;
    r->main->send_last_chain = 0;    
    r->connection->ssl_tailer_sent = 0;
    r->main->req_unfree = 0;
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    for ( i = 0; i < 10; i++)
    {
        s_ran_num[i]=rand()%10+'0';
        r->Content_Me[i] =s_ran_num[i];
    }
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                          "rand() num1: %*s", sizeof(s_ran_num), s_ran_num);

    len+=sizeof("Content-Me: ")-1 + sizeof(s_ran_num) + sizeof(CRLF)-1;
    
    if (NULL == r->headers_in.minictag) {
        // len+=sizeof("Referer-key: ")-1 + sizeof(R_ran) + sizeof(CRLF)-1;
        len+=sizeof("minictag: ")-1 + sizeof(r->main->Referrer) + sizeof(CRLF)-1;
    }
    ...
    *b->last++ = ' ';
    ...
    //b++
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"insert Content-Me");
    b->last = ngx_cpymem(b->last,"Content-Me: ", sizeof("Content-Me: ")-1);
    b->last = ngx_cpymem(b->last, s_ran_num, sizeof(s_ran_num));
    // b->last = ngx_cpymem(b->last, str->data, str->len);
    *b->last++ = CR; *b->last++ = LF;

    ngx_memset(r->content_me, 0, 16);
    ngx_memcpy(r->content_me, s_ran_num, sizeof(s_ran_num)); 


// add Referrer field
    if (NULL == r->headers_in.minictag) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"insert minitag");

        b->last = ngx_cpymem(b->last,"minictag: ", sizeof("minictag: ")-1);
        // b->last = ngx_cpymem(b->last,"minictag: ", sizeof("minictag: ")-1);//ֻ�ı�ʶ��������֮��ľͲ�����
        b->last = ngx_cpymem(b->last, r->main->Referrer, sizeof(r->main->Referrer));
        // b->last = ngx_cpymem(b->last, str->data, str->len);
        *b->last++ = CR; *b->last++ = LF;
        *b->last++ = CR; *b->last++ = LF;
    } else {
    ...
    //
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http proxy header:len(%z)%N\"%*s\"",
                   (size_t) (b->last - b->pos), (size_t) (b->last - b->pos), b->pos);
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
            ...
            if (u->headers_in.status_n == NGX_HTTP_SWITCHING_PROTOCOLS) {
                ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "proxy_process_set_u->keepalive = 0");
                u->keepalive = 0;
...
ngx_http_proxy_non_buffered_chunked_filter(void *data, ssize_t bytes)
{
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log,0,"chunk_input_filter r =%d",r->key);

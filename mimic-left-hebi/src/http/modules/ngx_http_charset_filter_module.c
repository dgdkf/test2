...
//    if (r == r->main) {//zdc906
      if (1) {//zdc906
        ...
        charset = ngx_http_destination_charset(r, &dst);//main

    } else {
        charset = ngx_http_main_request_charset(r, &dst);//sub
    }
    ...
    if (charset == NGX_DECLINED) {
        return ngx_http_next_header_filter(r);//sub && main
    }
...
ngx_http_charset_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if(r->upstream!=NULL){
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "ext: %V---http upstream request line: \"%V\",r->key=%d",r->upstream->peer.name, &r->request_line,r->key);
    }     


    ngx_flow_node_t  *fl_nd;   
    ngx_chain_t *vt_out=NULL,*bdc=NULL,*din=NULL;
    size_t      valid_tailer = 0;
    ngx_int_t   count_flnd = 0;

    if(r->send_last_chain== 1){
    	ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"--------last chain---of-----%V",r->upstream->peer.name);
    ...
    for ( fl_nd = r->main->flow_node ; fl_nd ; fl_nd = fl_nd->next){
        ...
        if(fl_nd->last_flag == 1){
            valid_tailer++;
        }
    }

    if(count_flnd < r->main->mirror_num) {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"----other flow_node need to be created-----");
        return NGX_ERROR;
    }

 	ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"valid_tailer=%i,mirror_num=%i,send_again=%d",
						                            valid_tailer,r->main->mirror_num,r->main->send_again);
    if(r->headers_out.status >= 500){
        r->send_last_chain = 1;
    }
    ...
    if(fl_nd ==  NULL){
        ngx_log_error(NGX_LOG_NOTICE, r->connection->log, 0,"----fl_nd==NULL--r->key:%i----",r->key);
        return NGX_ERROR;
    }

    /*-------------------------------------------------------------------*/
    if(r->exist_body == 1){
        for ( din = in ; din ; din = din->next)  {

            if(din==NULL || din->buf==NULL || din->buf->last - din->buf->pos == 0 ){
                break;
            }

            //申请一个带buf的cl
            bdc = abt_get_chain_buf(r,fl_nd);
            if(bdc){
                bdc->buf->last = ngx_cpymem(bdc->buf->last, din->buf->pos, ngx_buf_size(din->buf));
                fl_nd->size += ngx_buf_size(din->buf);
            }
        }

        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"store_body=%l",fl_nd->size);
    }
    ...
    if(r->main->aga_chain) {  
        rc = ngx_http_next_body_filter(r,r->main->aga_chain);
        return rc;
    }
    ...
    if(1){
        ...
        for ( fl_nd = r->main->flow_node ; fl_nd ; fl_nd = fl_nd->next){
            ...
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"#####body:%d abt len=%d",fl_nd->key,fl_nd->size);
        }    

        if(r->main->self_e_page == 1 ) {
            
            r->main->tailer_sent = 1;
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"---co_ready---r->main->need_body == 0"); 
            return NGX_OK;
        } 

        if(r->main->select_en){
            for( fl_nd = r->main->flow_node;fl_nd;fl_nd=fl_nd->next ) {
                if(fl_nd->key == r->main->select_key && r->main->select_abt == 0){
                    if(fl_nd->last_flag == 1){

                        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                                    "#######--select body: %V :%ui---size:%ui---b_out_len:%ui,tailer:%d",
                                    fl_nd->name,fl_nd->key,fl_nd->size,r->main->b_out_len,r->main->tailer_sent);

                        r->main->b_out_len = fl_nd->size;
                        vt_out =  fl_nd->body_data_chain;
                        r->main->select_abt = 1;
            ...
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"tailer_sent=%d  pre_abt_done=%d",
                                                                    r->main->tailer_sent,r->main->pre_abt_done);

            if(valid_tailer == 2 && r->main->pre_abt_done == 0){
                ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"bodys voting (2)");

                vt_out = mimic_bodys_voting(r,2);
                if(vt_out == NULL) {

                    if( r->main->mirror_num == 2 ){
                        if(PRESTIGE_ON){
                            get_share_prestige(r);

                            for( fl_nd = r->main->flow_node;fl_nd;fl_nd=fl_nd->next ) {
                                if(fl_nd->key == r->main->select_key) {
                                    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"select body:%d",r->main->key);
                                    vt_out = fl_nd->body_data_chain;
                                    r->main->pre_abt_done = 1;
                                }
                            }
            ...
            }else if(valid_tailer == 3 && r->main->abt_done == 0){
                ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"bodys voting (3)");

                r->main->abt_done = 1;
                vt_out = mimic_bodys_voting(r,3);
                if(vt_out == NULL) {
        ...
        if(vt_out != NULL) {
            ...
            rc = ngx_http_next_body_filter(r, vt_out);  
            
            if(r->main->send_again){                       
                return rc;
            }
        ...
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"tailer_sent=:%d",r->main->tailer_sent);

        if( valid_tailer == 2 && r->main->tailer_sent == 0 ) {     //当前两个异常，最后一个到了怎么办
            ngx_buf_t *lb;
            ngx_chain_t *lc;
            ...
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "body last (2)");

            r->main->ready_tailer = 1;
            
            rc = ngx_http_next_body_filter(r, lc);
	    if(r->connection->ssl_tailer_sent == 1){
		r->main->tailer_sent = 0;
	    }
            return NGX_OK;
        }

        if(valid_tailer == 3 && r->main->send_again == 0) {
    ...
    return rc; //zdc 20210322 DO: disable the function of charset 
    

// output: //add goto syntax---zdc
//----------------------------------------------------------------------------------*/
    ...
    if (ctx == NULL || ctx->table == NULL) {
        return ngx_http_next_body_filter(r, in);//sub1010
    }

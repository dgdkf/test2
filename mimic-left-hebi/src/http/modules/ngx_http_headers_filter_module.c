...
ngx_http_headers_filter(ngx_http_request_t *r)
{
    ...
    /*if (r != r->main) {
        return ngx_http_next_header_filter(r);//sub
    }*///zdc906
...
static ngx_int_t ngx_http_headers_filter_request_handler(ngx_http_request_t *r){
    ngx_uint_t                      i;
    ...
    for(i = 0; ; i++){
        ...
        if( 0 == ngx_strncasecmp(header[i].key.data, (u_char*)"Cookie",header[i].key.len)){
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "ngx_http_headers_filter_request_handler %ud \"%V \"",i,
                           &header[i].key);
            if(ngx_strncmp(r->uri.data,WHITELIST1,r->uri.len) == 0  ||\
                ((r->request_line.len >= 33) && (r->method == NGX_HTTP_POST) &&\
                (ngx_strncmp(r->request_line.data + 5, WHITELIST1, r->request_line.len - 5 - 9) == 0))){
                ngx_str_set(&header[i].key, "TMPCOK");//让每次登录响应都生成session
            }
        }
    }
    return NGX_DECLINED;
...
ngx_http_headers_filter_init(ngx_conf_t *cf)
{
    ...
    *h = ngx_http_headers_filter_request_handler;

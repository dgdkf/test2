...
//        || r != r->main  //zdc 906
        ...
        || r->expect_trailers)
    {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                            "r=%d,r->headers_out.content_len_n=%d,trailers = %d",
                            r->key,r->headers_out.content_length_n,r->expect_trailers);
            ...
            && clcf->chunked_transfer_encoding)
        {
            ...
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, 
                                                "set chunk r=%d,r->chunk=%d",r->key,r->chunked);
            r->chunked = 1;
...
ngx_http_chunked_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                                "r->key%i   chunk = %d",
                                    r->key,r->main->header_chunk);

    if (in == NULL || !r->main->header_chunk || r->header_only || r->main->send_again) {
        return ngx_http_next_body_filter(r, in);
    }
    
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"------chunked body encoding-----");
    ...
    if (cl->buf->last_buf) {
        ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,"------ngx_http_chunked_create_trailers-----");

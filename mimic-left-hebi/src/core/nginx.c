...
int get_local_macs(ngx_cycle_t      *cycle)
{
  int sock;
  int i = 0, ifCount = 0;
  int code = 0;
  ...
  if (sock == -1)
  {
	ngx_log_debug0(NGX_LOG_DEBUG_CORE, cycle->log, 0,"-perror(socket)-");
    //perror("socket");
    return -1;
  }
  ...
  ifc.ifc_len = 3000;
  ifc.ifc_buf = (void*)pBuffer;
 
  code = ioctl(sock, SIOCGIFCONF, &ifc);
  ngx_log_debug3(NGX_LOG_DEBUG_CORE, cycle->log, 0,
                 "-ioctl return %d, ifc_len:3000->%d, ifr size:%d-",
                 code, ifc.ifc_len, (int)sizeof(struct ifreq));
  if (code < 0)
  {
    free(pBuffer);
    close(sock);
    ngx_log_debug0(NGX_LOG_DEBUG_CORE, cycle->log, 0, "-perror(ioctl)-");
   // perror("ioctl");
    return -1;
  }
  ...
  for(i = 0; i < ifCount; ++i) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_CORE, cycle->log, 0, "-%d:%s:-",
                   i, ifr.ifr_name);
    if(0 == code){
        ngx_log_debug1(NGX_LOG_DEBUG_CORE, cycle->log, 0,
                       "-flags:%xd:-", ifr.ifr_flags);
    }else{
        ngx_log_debug0(NGX_LOG_DEBUG_CORE, cycle->log, 0, "-flagserror-");
    }
 
    code = ioctl(sock, SIOCGIFHWADDR, &ifr);
    if(0 == code){
        puc = (unsigned char *)ifr.ifr_hwaddr.sa_data;
        if(puc[0] == 0 && puc[1] == 0 && puc[2] == 0 && puc[3] == 0 && puc[4] == 0 && puc[5] == 0){
            continue;
        }else{
            ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0,
                          "-mac:%xd-%xd-%xd-%xd-%xd-%xd-",
                          puc[0],puc[1], puc[2], puc[3], puc[4], puc[5]);
            ngx_board_id = ((puc[3]-1)<<7) | ((puc[4]-1)<<5 );
            break;
        }
    }else{
        ngx_log_debug0(NGX_LOG_DEBUG_CORE, cycle->log, 0, "-mac:errrrrrrrr-");
    }
  }
...
main(int argc, char *const *argv)
{
    ...
    //get local mac
    get_local_macs(cycle);
    ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0,
                  "-main--ngx_board_id: \"%xd\"", (int)ngx_board_id);
    ...
    key = ftok("/usr", 0666);
    shmid = shmget(key,sizeof(shm_prestige_t),0666|IPC_CREAT);
    if(shmid == -1){
        shmctl(shmid, IPC_RMID, 0);
        shmid = shmget(key,sizeof(shm_prestige_t),0666|IPC_CREAT);
        if(shmid == -1){
            ngx_write_stdout("shm failed ");
            return 0;
        }
    }
    ...
    pthread_mutex_init(&shared->mutex,NULL);    //pthread_mutex_destroy
    shmdt(shm);
    ...
    if (ngx_show_help) {
        ngx_write_stderr(
            ...
            "  -s signal     : send signal to a master process: "
                               "stop, quit, reopen, reload, remimic" NGX_LINEFEED
                            //    "stop, quit, reopen, reload" NGX_LINEFEED//zdczdc20200908
#ifdef NGX_PREFIX
        ...
        while (*p) {
            ...
            case 's':
                ...
                if (ngx_strcmp(ngx_signal, "stop") == 0
                    ...
                    || ngx_strcmp(ngx_signal, "remimic") == 0

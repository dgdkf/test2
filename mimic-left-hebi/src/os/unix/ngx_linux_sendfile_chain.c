...
eintr:
    ...
    if (n == -1) {
        ...
        case NGX_EAGAIN:
            ...
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0,"[[[sendfile eagain]]]");
            return NGX_AGAIN;

        case NGX_EINTR:
            ngx_log_debug0(NGX_LOG_DEBUG_EVENT, c->log, err,
                           "sendfile() was interrupted");
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0,"[[[sendfile eintr]]]");
            goto eintr;

        default:
            c->write->error = 1;
            ngx_connection_error(c, err, "sendfile() failed");
            ngx_log_debug(NGX_LOG_DEBUG_HTTP, c->log, 0,"[[[sendfile failed]]]");
            return NGX_ERROR;
        }
    }

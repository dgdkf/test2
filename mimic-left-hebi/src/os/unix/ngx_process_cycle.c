...
ngx_uint_t    ngx_exiting;      //nginx���������˳�������
sig_atomic_t  ngx_reconfigure;  //���¼��������ļ���Ҳ����reload
sig_atomic_t  ngx_reopen;       //���´������ļ�
//zdczdc20200908
sig_atomic_t  ngx_remimic;       //���²þ�ִ�����б�
...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    //zdczdc20200908
    sigaddset(&set, ngx_signal_value(NGX_REMIMIC_SIGNAL));
    ...
    for ( ;; ) {
        ...
        if ( ngx_remimic ){
            ngx_remimic = 0;
            ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0, "remimicing");
            ngx_signal_worker_processes(cycle,
                                       ngx_signal_value(NGX_REMIMIC_SIGNAL));
        }
...
ngx_single_process_cycle_loop(void *arg)
{
	...
	//zdczdc20200908
        if ( ngx_remimic ){
            ngx_remimic = 0;
            ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0, "worker----------remimicing");
            // ngx_signal_worker_processes(cycle,
            //                            ngx_signal_value(NGX_REMIMIC_SIGNAL));
            FILE *fp;
            char buffer[64];
            if((fp=fopen((char *)cycle->conf_file.data,"r"))==NULL)
            {
                ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0, "open file failed during remimicing");
                return 0;
            }
            while(!feof(fp))
	        {
	    	    if(fgets(buffer,sizeof(buffer),fp)!=NULL)
	            {
                    ...
                    {
                        ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0, "---remimic_content---%s",buffer);
                        cycle->abt_extor = ngx_palloc(cycle->pool,sizeof(buffer));
                        ngx_cpystrn(cycle->abt_extor,(u_char *)buffer,sizeof(buffer));
                        return 0;
                    }
                }
            }           
...
ngx_signal_worker_processes(ngx_cycle_t *cycle, int signo)
{
    ...
    case ngx_signal_value(NGX_REOPEN_SIGNAL):
        ...
        break;
    //zdczdc20200908
    case ngx_signal_value(NGX_REMIMIC_SIGNAL):
        ch.command = NGX_CMD_REMIMIC;
        break;
